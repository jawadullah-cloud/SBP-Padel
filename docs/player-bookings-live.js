(()=>{
  ...
  async function load(){
    ...
    state('Loading your bookings…');
    try{
      const rows=await api(`/bookings/me?_=${Date.now()}`);
      const venues=new Map();
      const venueDetails=new Map();
      for(const id of [...new Set(rows.map(x=>x.venue_id))]){
        try{const d=await api(`/venues/${id}`);venueDetails.set(id,d);venues.set(id,d)}catch{}
      }
      const details=await Promise.all(rows.map(async b=>{try{return {...b,detail:await api(`/bookings/${b.id}`)}}catch{return {...b,detail:null}}}));
      const today=new Date();today.setHours(0,0,0,0);
      const groups={upcoming:[],past:[],cancelled:[]};
      for(const b of details){
        const day=new Date(`${b.date}T12:00:00`);
        if(['cancelled','venue_cancelled'].includes(b.status))groups.cancelled.push(b);
        else if(['completed','expired','payment_failed'].includes(b.status)||day<today)groups.past.push(b);
        else groups.upcoming.push(b);
      }
      const card=(b,type)=>{
        const d=b.detail||{},vd=venueDetails.get(b.venue_id),court=vd?.courts?.find(c=>c.id===b.court_id),first=d.slots?.[0],last=d.slots?.[d.slots.length-1];
        const time=first?`${fmtTime(first.start_time)}${last?` – ${fmtTime(last.end_time)}`:''}`:'';
        const label=type==='cancelled'?'CANCELLED':type==='past'?(b.status==='completed'?'COMPLETED':statusLabel(b.status)):statusLabel(b.status);
        return `<article class="bkCard" data-booking-card="${esc(b.id)}"><div class="bkHero"><span class="bkState ${type==='past'?'past':type==='cancelled'?'cancelled':''}">${esc(label)}</span></div><div class="bkBody"><div class="bkTop"><div><small>${esc((vd?.name||'SBP PADEL').toUpperCase())}</small><h2>${esc(court?.name||'Court')} · ${esc(court?.court_type||'Padel Court')}</h2><small>${esc(fmtDate(b.date))}</small></div><div class="bkTime">${esc(time)}</div></div><div class="bkMeta"><div><small>Booking ID</small><b>${esc(b.booking_code)}</b></div><div><small>Amount</small><b>${money(b.total)}</b></div></div><div class="bkActions">${type==='upcoming'?`<button class="bkPrimary" data-live-manage="${esc(b.id)}">MANAGE BOOKING</button><button class="bkSecondary" data-live-pass="${esc(b.id)}">PASS</button>`:`<button class="bkPrimary" data-live-rebook="${esc(b.id)}">BOOK AGAIN</button><button class="bkSecondary" data-live-manage="${esc(b.id)}">DETAILS</button>`}</div></div></article>`;
      };
      root.innerHTML=`<div class="bkWrap"><div class="bkHead"><div><h1>My Bookings</h1><p>Manage upcoming and previous court sessions.</p></div><div class="bkAvatar">${esc(initials())}</div></div><div class="bkTabs"><button class="on" data-live-tab="upcoming">Upcoming (${groups.upcoming.length})</button><button data-live-tab="past">Past (${groups.past.length})</button><button data-live-tab="cancelled">Cancelled (${groups.cancelled.length})</button></div>${['upcoming','past','cancelled'].map(g=>`<div class="bkList" id="live-${g}" ${g!=='upcoming'?'hidden':''}>${groups[g].length?groups[g].map(x=>card(x,g)).join(''):`<div style="padding:34px 12px;text-align:center;color:var(--muted);font-size:9px">No ${g} bookings.</div>`}</div>`).join('')}</div>`;
      root.querySelectorAll('[data-live-tab]').forEach(btn=>btn.onclick=()=>{root.querySelectorAll('[data-live-tab]').forEach(x=>x.classList.toggle('on',x===btn));['upcoming','past','cancelled'].forEach(g=>{const list=document.getElementById(`live-${g}`);if(list)list.hidden=g!==btn.dataset.liveTab})});
      root.querySelectorAll('[data-live-pass]').forEach(btn=>btn.onclick=async e=>{e.preventDefault();e.stopPropagation();const b=details.find(x=>x.id===btn.dataset.livePass);if(!b?.detail)return;const vd=venueDetails.get(b.venue_id),court=vd?.courts?.find(c=>c.id===b.court_id);const p={venue:vd?.name||'SBP Padel',court:court?.name||'Court',courtType:court?.court_type||'',date:fmtDate(b.date),dateIso:b.date,slots:(b.detail.slots||[]).map(s=>`${fmtTime(s.start_time)} – ${fmtTime(s.end_time)}`),slotStarts:(b.detail.slots||[]).map(s=>s.start_time),amount:Number(b.total)};localStorage.setItem('sbpPadelPayment',JSON.stringify(p));localStorage.setItem('sbpPadelBookingId',b.booking_code);localStorage.setItem('sbpPadelSelectedBookingId',b.id);window.SBPDeepRoute?window.SBPDeepRoute('digital-pass.html'):window.SBPNavigate?.('pass')});
      return rows;
    }catch(err){state(`Could not load bookings: ${err.message}`);return[]}
  }

  window.SBPRefreshBookings=load;
  document.addEventListener('click',e=>{if(e.target.closest?.('[data-nav="bookings"]'))setTimeout(load,0)},true);
  window.addEventListener('focus',()=>{if(root.classList.contains('active'))load()});
