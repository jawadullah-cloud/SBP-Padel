(()=>{
  ...
  let venues=[],detail=null,detailSeq=0;
  async function api(path){const r=await fetch(`${apiBase()}${path}`,{cache:'no-store'});let b=null;try{b=await r.json()}catch{}if(!r.ok)throw new Error(typeof b?.detail==='string'?b.detail:`Request failed (${r.status})`);return b}
  ...
  function openExternal(url){if(!url)return;try{if(window.SBPAndroid&&typeof window.SBPAndroid.openExternal==='function'){window.SBPAndroid.openExternal(url);return}}catch{}location.href=url}
  ...
  function renderLoading(v){const root=document.getElementById('nishtar');if(!root)return;root.innerHTML=`<div class="venueRefHero courtScene"><button class="back" type="button" data-sbp-venue-back aria-label="Back to venues">←</button></div><div class="venueRefBody"><div class="venueTitleRow"><div><p class="overline">SPORTS BOARD PUNJAB</p><h1>${esc(v?.name||'Loading venue…')}</h1></div></div><section class="venueRefSection"><p>Loading venue details…</p></section></div>`;window.SBPRefreshBackIcons?.()}
  ...
  window.SBPPlayerOpenExternal=openExternal;
  load();
})();
