(()=>{
  ...
  function newBooking(){const keep={venueId:state.venueId,venueName:state.venueName};state={...fresh(),...keep,date:todayISO(),courtId:null,courtName:null,courtType:null};localStorage.removeItem('sbpPadelBookingDatePicker');save()}
  ...
  function goMain(screen,focusStep){
    ...
    if(window.SBPNavigate){window.SBPNavigate(screen);setTimeout(()=>focusSection(focusStep),50);return}
  ...
  function decorateSteps(){const roots=[...document.querySelectorAll('#select .steps,#time .steps')];if(PATH==='review-booking.html')roots.push(...document.querySelectorAll('.progress'));roots.forEach(root=>{[...root.querySelectorAll(':scope > b,:scope > span')].forEach((el,i)=>{const n=i+1;if(n>5)return;el.dataset.sbpStep=String(n);el.setAttribute('role','button');el.setAttribute('tabindex','0');el.style.cursor='pointer';el.style.userSelect='none';el.title=`Go to step ${n}`})})}

  let venueDetail=null,availability=null,availabilitySeq=0,syncTimer=null;
  async function bootstrapData(){
    const venues=await api('/venues');if(!venues.length)throw new Error('No active venues are configured.');
    const venue=venues.find(v=>v.id===state.venueId)||venues[0];state.venueId=venue.id;state.venueName=venue.name;
    venueDetail=await api(`/venues/${venue.id}`);if(!state.date||state.date<todayISO())state.date=todayISO();save();
  ...
  function dateButtons(){
    const rail=document.querySelector('#select .dateRail');if(!rail)return;ensureDateStyle();const base=new Date();rail.innerHTML='';let quickSelected=false;
  ...
  function renderCourts(){const list=document.querySelector('#select .courtList');if(!list||!availability)return;list.innerHTML=availability.courts.map(c=>`<article class="courtOption ${c.court_id===state.courtId?'selected':''}" data-court="${esc(c.court_id)}"><div class="courtThumb"></div><div><h4>${esc(c.court_name)}</h4><p>${esc(c.court_type)}</p></div><span class="${c.court_id===state.courtId?'courtTick':'courtCap'}">${c.court_id===state.courtId?'✓':'4'}</span></article>`).join('')}
  function selectedCourt(){return state.courtId?availability?.courts?.find(c=>c.court_id===state.courtId)||null:null}
  function reconcileSlots(){const c=selectedCourt();if(!c)return 0;const allowed=new Set(c.slots.filter(s=>s.available).map(s=>s.start_time));const before=state.slotStarts.length;state.slotStarts=state.slotStarts.filter(s=>allowed.has(s));const removed=before-state.slotStarts.length;if(removed){state.quote=null;state.policyAccepted=false;save()}return removed}
  function renderTimes(){const list=document.querySelector('#time .timeline');const c=selectedCourt();if(!list)return;if(!c){list.innerHTML='<div style="padding:22px 10px;text-align:center;color:var(--muted);font-size:9px">Select a court to view live availability.</div>';const sub=document.querySelector('#time .subline');if(sub)sub.textContent=fmtDate(state.date);refreshSummary();return}reconcileSlots();const sub=document.querySelector('#time .subline');if(sub)sub.textContent=`${fmtDate(state.date)} · ${state.courtName}`;list.innerHTML=c.slots.map(s=>{const chosen=state.slotStarts.includes(s.start_time)&&s.available;const unavailable=!s.available;return `<article class="slotRow ${unavailable?'booked':''} ${chosen?'chosen':''}" data-start="${esc(s.start_time)}" data-rate="${Number(s.hourly_rate||0)}"><div><b>${fmtTime(s.start_time)} – ${fmtTime(s.end_time)}</b><small>${unavailable?esc(s.unavailable_reason||'Unavailable'):chosen?'Selected':`${money(s.hourly_rate)} / hour`}</small></div><div style="text-align:right"><strong style="display:block;font:800 10px var(--sport);color:${unavailable?'var(--muted)':'var(--brand)'}">${s.hourly_rate?money(s.hourly_rate):'—'}</strong><button class="slotBook" ${unavailable?'disabled':''}>${unavailable?'UNAVAILABLE':chosen?'✓':'BOOK'}</button></div></article>`}).join('');refreshSummary()}
  ...
  function showTimeLoading(){const list=document.querySelector('#time .timeline');if(list)list.innerHTML='<div style="padding:22px 10px;text-align:center;color:var(--muted);font-size:9px">Checking live availability…</div>';const summary=document.getElementById('slotSummary'),total=document.getElementById('slotTotal');if(summary)summary.textContent='Refreshing slots';if(total)total.textContent='Please wait'}
  async function refreshAvailability(){
    if(!state.venueId||!state.date)return false;
    const seq=++availabilitySeq,venueId=state.venueId,date=state.date,courtId=state.courtId;
    const data=await api(`/venues/${venueId}/availability?date=${encodeURIComponent(date)}&_=${Date.now()}`);
    if(seq!==availabilitySeq||state.venueId!==venueId||state.date!==date)return false;
    availability=data;
    const c=state.courtId?availability.courts.find(x=>x.court_id===state.courtId)||null:null;
    if(c){state.courtName=c.court_name;state.courtType=c.court_type}
    else if(courtId&&state.courtId===courtId){state.courtId=null;state.courtName=null;state.courtType=null;state.slotStarts=[];state.quote=null;save()}
    renderCourts();renderTimes();return true;
  }
  async function ensureQuote(){if(!state.venueId||!state.courtId||!state.date||!state.slotStarts.length)throw new Error('Booking selection is incomplete.');const q=await api('/bookings/quote',{method:'POST',body:JSON.stringify({venue_id:state.venueId,court_id:state.courtId,booking_date:state.date,slots:state.slotStarts.map(start_time=>({start_time}))})});state.quote=q;state.venueName=q.venue?.name||state.venueName;state.courtName=q.court?.name||state.courtName;state.courtType=q.court?.court_type||state.courtType;save();return q}

  async function openTime(){if(!state.courtId){toast('Choose a court first.',true);goMain('select',3);return}goMain('time',4);showTimeLoading();try{await refreshAvailability()}catch(e){toast(e.message,true)}}
  async function stepTo(n){if(n===1){goMain('nishtar',1);return}if(n===2||n===3){goMain('select',n);return}if(n===4){await openTime();return}if(n===5){if(!state.slotStarts.length){toast('Select at least one available time slot first.',true);goMain('time',4);return}try{await ensureQuote();goDeep('review-booking.html')}catch(e){toast(e.message,true)}}}
  ...
  function installMainCapture(){
    document.addEventListener('click',async e=>{
      ...
      const start=e.target.closest?.('#nishtar [data-nav="select"],#home [data-nav="select"]');if(start){e.preventDefault();e.stopPropagation();e.stopImmediatePropagation();reloadState();if(state.status==='confirmed')newBooking();dateButtons();renderCourts();goMain('select',2);refreshAvailability().then(()=>{dateButtons();renderCourts()}).catch(err=>toast(err.message,true));return}
      ...
      const toTime=e.target.closest?.('#select .bookingBottom .primary');if(toTime){e.preventDefault();e.stopPropagation();e.stopImmediatePropagation();await openTime();return}
      ...
      const review=e.target.closest?.('#time .bookingBottom .primary');if(review){e.preventDefault();e.stopPropagation();e.stopImmediatePropagation();if(review.dataset.busy==='1')return;if(!state.slotStarts.length){toast('Select at least one available time slot.',true);return}review.dataset.busy='1';const old=review.innerHTML;review.innerHTML='CHECKING AVAILABILITY…';const chosen=[...state.slotStarts];try{await refreshAvailability();const removed=chosen.filter(x=>!state.slotStarts.includes(x));if(removed.length){toast('That slot has just become unavailable. Please choose another time.',true);return}await ensureQuote();goDeep('review-booking.html')}catch(err){toast(err.message,true)}finally{review.dataset.busy='0';review.innerHTML=old}return}
    },true);
  ...
  async function syncFromStorage(){
    ...
    if(state.status==='confirmed'||state.status==='booking_created'){decorateSteps();return}
  ...
  function scheduleSync(){clearTimeout(syncTimer);syncTimer=setTimeout(()=>{if(document.querySelector('#select.active,#time.active'))syncFromStorage()},120)}
  async function mainPage(){
    installMainCapture();window.SBPBookingFlowSync=syncFromStorage;
    ...
    window.addEventListener('pageshow',scheduleSync);window.addEventListener('focus',scheduleSync);document.addEventListener('visibilitychange',()=>{if(!document.hidden)scheduleSync()})
  }
  ...
  async function reviewPage(){
    reviewStyle();decorateSteps();if(!state.quote){try{await ensureQuote()}catch(e){toast(e.message,true);setTimeout(()=>goMain('time',4),700);return}}
    const q=state.quote,user=await api('/auth/me'),policy=await api('/policies/active');state.policyId=policy.id;state.policyTitle=policy.title;state.policyBody=policy.body;save();const set=(id,val)=>{const el=document.getElementById(id);if(el)el.textContent=val};set('venue',state.venueName);set('court',`${state.courtName}${state.courtType?` · ${state.courtType}`:''}`);set('date',fmtDate(state.date));set('time',(q.slots||[]).map(s=>`${fmtTime(s.start_time)} – ${fmtTime(s.end_time)}`).join(', '));set('feeLabel',`Court fee × ${state.slotStarts.length} hour${state.slotStarts.length===1?'':'s'}`);set('fee',money(q.court_fee));const priceRows=document.querySelectorAll('.priceRow');if(priceRows[1]?.querySelector('b'))priceRows[1].querySelector('b').textContent=money(q.service_fee);set('total',money(q.total));const owner=document.querySelector('.playerCard h3');if(owner)owner.textContent=user.full_name;const av=document.querySelector('.playerCard .avatar');if(av)av.textContent=String(user.full_name||'P').split(/\s+/).slice(0,2).map(x=>x[0]).join('').toUpperCase();const old=document.getElementById('policyCard')||document.getElementById('livePolicy');if(old)old.outerHTML=`<section id="livePolicy"><div class="lpHead"><b>${esc(policy.title)}</b><br><small>Version ${esc(policy.version)}</small></div><div class="lpBody">${esc(policy.body)}</div><label><input id="livePolicyAccept" type="checkbox" ${state.policyAccepted?'checked':''}><span>I have read and accept the Booking, Cancellation & Refund Policy.</span></label></section>`;const cb=document.getElementById('livePolicyAccept'),btn=document.getElementById('toPayment');if(!cb||!btn)return;const paint=()=>{btn.disabled=!cb.checked;btn.setAttribute('aria-disabled',String(!cb.checked))};paint();cb.onchange=()=>{state.policyAccepted=cb.checked;save();paint()};btn.onclick=e=>{e.preventDefault();e.stopPropagation();if(!cb.checked)return;state.policyAccepted=true;state.status='reviewed';save();goDeep('payment.html')};const back=document.querySelector('.head .back');if(back)back.onclick=e=>{e.preventDefault();goMain('time',4)};document.addEventListener('click',e=>{const st=e.target.closest?.('[data-sbp-step]');if(st){e.preventDefault();e.stopPropagation();e.stopImmediatePropagation();stepTo(Number(st.dataset.sbpStep))}},true)
  }
  ...
  async function paymentPage(){
    ...
    const q=state.quote,set=(id,val)=>{const el=document.getElementById(id);if(el)el.textContent=val};set('venue',state.venueName);set('court',state.courtName);set('date',fmtDate(state.date));set('time',(q.slots||[]).map(s=>`${fmtTime(s.start_time)} – ${fmtTime(s.end_time)}`).join(', '));['amount','total','buttonAmount'].forEach(id=>set(id,money(q.total)));document.querySelectorAll('.payCard').forEach(card=>{card.classList.toggle('selected',card.dataset.method===state.paymentMethod);card.onclick=()=>{document.querySelectorAll('.payCard').forEach(x=>x.classList.remove('selected'));card.classList.add('selected');state.paymentMethod=card.dataset.method;save()}});const back=document.querySelector('.head .back');if(back)back.onclick=e=>{e.preventDefault();goDeep('review-booking.html')};const btn=document.getElementById('payButton');if(!btn)return;btn.onclick=async e=>{e.preventDefault();if(btn.disabled)return;btn.disabled=true;const label=btn.querySelector('span');if(label)label.textContent='PROCESSING…';try{await ensureQuote();const created=await api('/bookings',{method:'POST',body:JSON.stringify({venue_id:state.venueId,court_id:state.courtId,booking_date:state.date,slots:state.slotStarts.map(start_time=>({start_time})),policy_version_id:state.policyId,policy_accepted:true})});state.bookingUuid=created.id;state.bookingCode=created.booking_code;state.status='booking_created';save();const init=await api('/payments/initiate',{method:'POST',body:JSON.stringify({booking_id:created.id,method:state.paymentMethod||'wallet'})});state.paymentUuid=init.payment_id;save();const paid=await api(`/payments/${init.payment_id}/simulate-success`,{method:'POST'});state.paymentStatus=paid.payment_status;state.status='confirmed';save();localStorage.setItem('sbpPadelNotificationsVersion',String(Date.now()));try{window.parent?.SBPRefreshNotifications?.()}catch{}try{await window.parent?.SBPBookingFlowSync?.()}catch{}goDeep('payment-success.html')}catch(err){btn.disabled=false;if(label)label.textContent='PAY & CONFIRM';if(err.status===409||/available|booked|conflict|past|started/i.test(err.message)){state.slotStarts=[];state.quote=null;state.policyAccepted=false;state.status='selecting';save();toast(err.message||'That slot is no longer available.',true);setTimeout(()=>goMain('time',4),1000)}else toast(err.message,true)}}
  }

  function successPage(){if(!state.bookingCode)return;const q=state.quote||{},set=(id,val)=>{const el=document.getElementById(id);if(el)el.textContent=val};set('venue',state.venueName);set('court',state.courtName);set('date',fmtDate(state.date));set('time',(q.slots||[]).map(s=>`${fmtTime(s.start_time)} – ${fmtTime(s.end_time)}`).join(', '));set('amount',money(q.total));set('bookingId',state.bookingCode);set('method',state.paymentMethod==='card'?'Card':state.paymentMethod==='bank'?'Online Banking':'SBP Padel Wallet');localStorage.setItem('sbpPadelBookingId',state.bookingCode);syncLegacy();const pass=document.getElementById('viewPass');if(pass)pass.onclick=e=>{e.preventDefault();goDeep('digital-pass.html')};const bookings=document.getElementById('backHome');if(bookings){bookings.textContent='MY BOOKINGS';bookings.onclick=e=>{e.preventDefault();goMain('bookings')}}}
  function boot(){if(PATH==='index.html'||PATH==='')mainPage();else if(PATH==='review-booking.html')reviewPage().catch(e=>toast(e.message,true));else if(PATH==='payment.html')paymentPage().catch(e=>toast(e.message,true));else if(PATH==='payment-success.html')successPage()}
