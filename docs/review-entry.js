(()=>{
  ...
  async function refreshAvailability(){
    ...
    availability=data;
    let c=availability.courts.find(x=>x.court_id===state.courtId);
    if(!c)c=availability.courts.find(x=>x.court_id===courtId)||availability.courts[0]||null;
    if(c&&state.courtId!==c.court_id){state.courtId=c.court_id;state.courtName=c.court_name;state.courtType=c.court_type;save()}
    else if(c){state.courtName=c.court_name;state.courtType=c.court_type}
  ...
  async function stepTo(n){if(n===1){goMain('nishtar',1);return}if(n===2||n===3){goMain('select',n);return}if(n===4){await openTime();return}if(n===5){if(!state.slotStarts.length){toast('Select at least one time slot first.',true);goMain('time',4);return}try{await ensureQuote();goDeep('review-booking.html')}catch(e){toast(e.message,true)}}}
  ...
  function installMainCapture(){
    document.addEventListener('click',async e=>{
      ...
      const start=e.target.closest?.('#nishtar [data-nav="select"],#home [data-nav="select"]');if(start){e.preventDefault();e.stopPropagation();e.stopImmediatePropagation();reloadState();if(state.status==='confirmed')newBooking();goMain('select',2);refreshAvailability().catch(err=>toast(err.message,true));return}
