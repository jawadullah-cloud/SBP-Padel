(()=>{
  ...
  async function handleProviderCheckout(btn){
    if(btn.disabled)return;let state=load();const label=btn.querySelector('span');btn.disabled=true;if(label)label.textContent='PROCESSING…';try{
      ...
      if(booking&&['confirmed','rescheduled','completed'].includes(booking.status)){state.status='confirmed';save(state);go('payment-success.html');return}
      if(booking&&booking.status!=='pending_payment')throw new Error('This booking is no longer awaiting payment. Please select the slots again.');
      if(!booking){booking=await api('/bookings',{method:'POST',body:JSON.stringify({venue_id:state.venueId,court_id:state.courtId,booking_date:state.date,slots:(state.slotStarts||[]).map(start_time=>({start_time})),policy_version_id:state.policyId,policy_accepted:true})});state.bookingUuid=booking.id;state.bookingCode=booking.booking_code;state.status='booking_created';save(state)}
