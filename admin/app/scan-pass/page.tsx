...
export default function ScanPassPage(){
 const[token,setToken]=useState(''),[venues,setVenues]=useState<Venue[]>([]),[venueId,setVenueId]=useState(''),[value,setValue]=useState(''),[result,setResult]=useState<Validation|null>(null),[error,setError]=useState(''),[loading,setLoading]=useState(false),[cameraOn,setCameraOn]=useState(false),[cameraMessage,setCameraMessage]=useState('Camera is off. Use manual lookup or start the scanner.');
 const videoRef=useRef<HTMLVideoElement|null>(null),streamRef=useRef<MediaStream|null>(null),frameRef=useRef<number|null>(null),scanningRef=useRef(false),canvasRef=useRef<HTMLCanvasElement|null>(null);
 ...
 async function createNativeDecoder():Promise<((video:HTMLVideoElement)=>Promise<string>)|null>{
  const Detector=(window as Window&{BarcodeDetector?:BarcodeDetectorCtor}).BarcodeDetector;
  if(!Detector)return null;
  try{
   const detector=new Detector({formats:['qr_code']});
   return async(video:HTMLVideoElement)=>String((await detector.detect(video))?.[0]?.rawValue||'').trim();
  }catch{return null}
 }
 ...
 async function startCamera(){
  ...
  try{
   ...
   if(!video){stopCamera();throw new Error('Camera preview element is unavailable')}
   video.srcObject=stream;
   await video.play();
   setCameraOn(true);
   const nativeDecoder=await createNativeDecoder();
   const fallbackDecoder=createFallbackDecoder();
   setCameraMessage(nativeDecoder?'Camera live. Point it at the player’s SBP Padel QR code.':'Camera live. Using compatible QR decoder fallback.');
   const loop=async()=>{
    if(!streamRef.current)return;
    if(!scanningRef.current&&video.readyState>=2){
     scanningRef.current=true;
     try{
      const raw=nativeDecoder?await nativeDecoder(video):fallbackDecoder(video);
      if(raw){setValue(raw);setCameraMessage('QR detected. Validating…');await validate(raw);return}
     }catch(e){console.warn('QR decode frame failed',e)}finally{scanningRef.current=false}
    }
   ...
   stopCamera();
   const detail=cameraErrorText(e);
   setCameraMessage(`Camera failed: ${detail}`);
   setError(detail);
  }
 }
