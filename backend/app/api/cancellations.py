...
class CancelRequest(BaseModel):reason:str|None=Field(default=None,max_length=500)
...
async def cancel_booking(booking_id:UUID,payload:CancelRequest,user:User=Depends(current_user),db:AsyncSession=Depends(get_db))->dict:
    ...
    if booking.status not in {BookingStatus.pending_payment,BookingStatus.confirmed,BookingStatus.rescheduled}:raise HTTPException(409,f'A {booking.status.value} booking cannot be cancelled')
    policy=await booking_change_context(booking,db)
    if not policy['eligible']:raise HTTPException(409,policy['reason'])
    paid=booking.status in {BookingStatus.confirmed,BookingStatus.rescheduled};booking.status=BookingStatus.cancelled;booking.cancelled_at=datetime.now(timezone.utc);booking.cancellation_reason=payload.reason.strip() if payload.reason else None;refund=None;created_refund=False
    if paid:
        ...
        if payment:
            refund=await db.scalar(select(Refund).where(Refund.booking_id==booking.id,Refund.status.in_([RefundStatus.requested,RefundStatus.processing,RefundStatus.completed])))
            if not refund:
                refund=Refund(payment_id=payment.id,booking_id=booking.id,amount=payment.amount,currency=payment.currency,status=RefundStatus.requested,reason=booking.cancellation_reason or 'Player cancellation within refund policy');db.add(refund);await db.flush();created_refund=True
    ...
    if created_refund:db.add(Notification(user_id=user.id,kind='refund_requested',title='Refund requested',body=f'Refund processing has started for booking {booking.booking_code}.',payload={'booking_id':str(booking.id),'refund_id':str(refund.id)}))
    await db.commit();await slot_locks.release_booking(booking.id)
